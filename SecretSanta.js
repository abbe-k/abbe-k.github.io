...
SecretSanta.prototype.generate = function () {
    ...
    var candidateGenerator = function ( name ) {
    ...
    this.names.forEach( candidateGenerator, this );
    ...
    console.log("CandidateParirings ", candidatePairings)
    ...
    console.log("Parirings ", pairings)
    console.log("Blacklist ", this.blacklists)

    Object.entries(pairings).forEach( function([key, [value]]) {
        console.log(key)
        if (key in this.blacklists) {
            this.blacklists[key].push(value);
        }
        else {
            this.blacklists[key] = [value];
        }
    }, this);
    console.log("Blacklist After", this.blacklists)
    ...
    var getKeyByValue = function (object, value) {
        return Object.keys(object).find( function(key) {
            ...
            console.log(object[key].includes(value));
            return object[key].includes(value);
        });
    };


    var pairingsWithMore = Object.create( null );
    Object.entries(pairings).forEach( function ([name, [firstGift, secondGift]] ) {
        var clone = Object.assign({}, pairings);
        delete clone[name];
        console.log(clone);
        var firstGiftGiverToSame = getKeyByValue(clone, firstGift);
        var secondGiftGiverToSame = getKeyByValue(clone, secondGift);
        pairingsWithMore[name] = [[firstGift, firstGiftGiverToSame], [secondGift, secondGiftGiverToSame]];
    });
    console.log("pairingsWithMore ", pairingsWithMore);


    return pairingsWithMore;
